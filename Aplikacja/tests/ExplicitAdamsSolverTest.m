classdef ExplicitAdamsSolverTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testLinearOde(testCase)
            % Test 1: Simple equation without delay (dx/dt = x)
            % Solution: x(t) = exp(t)
            k = 2; h = 0.01; tk = 1;
            ...
            [time, sol] = explicitAdamsSolver(k, h, tk, f_ode, Xtau, dXtau, phi);

            analytical = exp(time);
            save('results_test.mat', 'sol', 'analytical');

            testCase.verifyEqual(sol, analytical, 'RelTol', testCase.TolSmall, ...
                'Solver can not handle a simple exponential equation - mall tolerance.');
        ...
        function testDdeDelay(testCase)
            % Test 2: Equation with a delay (DDE): dx/dt = -x(t-1)
        ...
        function testStepSizeConsistency(testCase)
            ...
            [~, sol1] = explicitAdamsSolver(k, 0.1, tk, f_ode, 0, 0, phi);
            [~, sol2] = explicitAdamsSolver(k, 0.01, tk, f_ode, 0, 0, phi);
